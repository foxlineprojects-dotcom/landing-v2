  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950 dark:text-white transition-colors">
      ...
      <main className="max-w-6xl mx-auto py-20 px-6">
        ...
        {/* Divider */}
        <div className="w-20 mx-auto h-1 bg-blue-600 dark:bg-blue-500 rounded-full mb-14"></div>
          ...
          {posts?.length > 0 ? (
            posts.map((p) => (
              <Link
                href={`/blog/${p.slug}`}
                key={p._id}
                className="group rounded-xl flex flex-col h-full overflow-hidden border border-slate-200 dark:border-slate-800 hover:shadow-xl hover:border-transparent dark:hover:border-transparent dark:hover:shadow-slate-700/30 transition-all duration-300 bg-white dark:bg-slate-900"
              >
                {/* Cover */}
                {p.coverImage && (
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <p className="mt-3 text-slate-600 dark:text-slate-400 line-clamp-3 flex-1">
                    {p.excerpt}
                  </p>

                  {/* CTA Bottom-Aligned */}
                  <span className="mt-5 inline-block text-sm font-medium text-blue-600 dark:text-blue-400 group-hover:underline">
                    Read More →
                  </span>
                </div>
              </Link>
